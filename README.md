...
app/webapp/routers/       misc (shell, /healthz, /api/version, /opener/opener.{cmd,ps1}) · auth (/login, /api/login|logout) · tasks
                          ...
                          · archive (/api/archive/run, /api/archive/runs[/{id}], per-item revert|move|accept)
...
app/webapp/static/        the PWA: index.html, login.html, styles.css (fleet tokens), app.js (state + routing), board.js,
                          ...
                          archive.js (the Archive tab: the batch run + its report), settings.js (the Settings tab),
...
| Row state | Accept | Move to… / File it… | Revert |
| --- | --- | --- | --- |
| `archived` · `moved` | – (finished, not reviewed) | into any *other* folder | deletes the files, mail back to the Inbox |
| `needs_review` | marks it seen (and teaches the folder it had) | files it for the first time — no undo leg | – (nothing on disk) |
| `failed` | marks it seen | re-files it | when files were written |
...
| State | What it means |
| --- | --- |
| `archived` | filed on disk and moved in Outlook — or already filed per the archiver's index, recorded with the existing file and nothing written |
