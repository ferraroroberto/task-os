...
The last manual chore: filing the Outlook Inbox one mail at a time. `POST /api/archive/run` does the whole tray in one go — every mail is written into an archive folder of [email-archiver](https://github.com/ferraroroberto/email-archiver)'s, moved to Outlook's *Archive* folder and tagged, and the run is recorded so you can look at what happened and undo any single mail. This is the backend (issues #157 and #158); the screen arrives in #159, so until then the trigger is the API.
...
`GET /api/status`'s `archive` key (and `tasks mirror status`) says whether this install can archive at all, what the last run did, and whether one is running now. Config changes need a restart — `tray.bat --restart` — like every other block here.
