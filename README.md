...
| `POST /api/tasks` | create → 201 (`title` + any task field; `due` and `starts` accept the natural phrases below, `""` clears) |
...
| `PATCH /api/tasks/{id}` · `DELETE /api/tasks/{id}` | partial update (fields present only; `parent_id` goes through the cycle guard; `due` and `starts` natural or ISO, `""` clears) · delete subtree |
...
- **frontmatter fields** `title`, `code`, `status`, `priority`, `due` and `starts` (ISO or a natural phrase — `tomorrow`, `next friday`, `oct 1`), `recurrence`, `person` (by name), `folder_ref`, `next_action`, `parent` (id; the cycle guard applies) and the **`## Description`** body are applied through the same repo layer as the UI, with **actor `md`** — every change lands in the activity log like any other;
