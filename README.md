...
| `voice` | `transcribe_url` — where a recorded quick-add line is forwarded (default `http://127.0.0.1:8000/v1/audio/transcriptions`, the [local-llm-hub](https://github.com/ferraroroberto/local-llm-hub); it names no model, so the hub applies its transcribe role — parakeet first); blank = voice off, visibly. `fallback_url` — the local whisper server, tried **only** when the hub cannot be reached at all (default `http://127.0.0.1:8090/v1/audio/transcriptions`; blank = no fallback). See [Voice quick-add](#voice-quick-add) |
...
| `POST /api/transcribe` (body = the audio, `Content-Type` = its type) | the recorded quick-add line → `{text, parse}` — the transcript **and** the `POST /api/parse` split of it, in one round trip. The body *is* the clip (no multipart), and what the browser sends is always 16 kHz mono WAV, which is the only thing either endpoint decodes. `409 voice_disabled` (no `voice.transcribe_url`) · `413 audio_too_large` · `422` (empty body) · `502 voice_rejected` (the endpoint answered and refused the clip, with what it said) · `503 voice_unavailable` (it never answered) — see [Voice quick-add](#voice-quick-add) |
...
Hands busy? Open the add dialog, **hold the mic beside line 1**, say *"buy a new filter for the dehumidifier next week"*, let go. The line fills with the transcript and the **Due** field already carries next week's date — the same [quick-add parse](#api) typing it would have produced — one confirming Enter from a task. Same gesture on the phone.
