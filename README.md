...
| `GET /api/today?person=` | `{today, plan: {items, done, total}, due: [{root, items}], week: [{root, items}], counts: {overdue, today, week}}` — `plan` = the tasks committed to today ordered by `plan_order`, done ones included for the progress line ([Plan my day](#plan-my-day)); `due` = open tasks due ≤ today grouped by root project (sorted by due date within each group), minus what is planned today; `week` = tomorrow … +7 days in the same shape (the CLI's shape; the web app derives the due/week buckets from the filtered list itself and reads only `plan` from here) |
