...
tasks triage            # stage one AI suggestion per Inbox task; y/n/q reviews them
...
- **One row, one filter card, every view** (#46) — Board, Table, Tree, Today and the Search tab's task hits are *renderings* of the same filtered list, drawn with the same row (`rows.js`): line 1 is the one-line ellipsized title with the **status select** right-aligned beside it (the one status control, on every row, mouse or touch); line 2 is the meta line — code (coding tasks) · project · due (relative, overdue in the danger tone) · priority marker · repeat glyph · folder chip · issue chip · children count · comment count (the drawer shows the comments, never the row) · person. **The date on that line is a control, not a label** (#107): click or tap it and the same picker the drawer's Due field opens comes up, so re-planning a task costs one gesture instead of opening the card and finding the field. It looks exactly as it did — only its tap area grew, and it, the folder chip and the AI chip are laid out so no two of their touch areas overlap. Flat hairlines between rows, no card boxes. Above every view sits a **top strip** — the live **text filter**, always on screen because it is the one reached for most, and the quick-add `+` (#80) — and under it the same **filter card** (`filters.js`), collapsed by default with its state spelled out on the summary line (`todo · Home renovation · sorted by due date · 12 tasks`): project, **person (multi-select — one click each, several allowed: *Anyone* / the name / *2 people*)**, due window, modified window (incl. its inverse — **untouched > 30/60/90 days**, the weekly stale pass: do it, defer it with a start date, or admit it's dead; any write counts as a touch, sync and mirror included, so a GitHub-synced task never looks stale), **status (multi-select, incl. `done` / `cancelled`, so finished items are findable)** and **sort**; on the phone the strip keeps the text box + `+` on one line and the card's controls sit two per line at equal widths (due · priority · last modified · created · title — the Tree's order comes from here too). The state **is the URL** — `?status=todo&project=12&sort=updated&updated=week` is the same shareable view on every tab; the default is open tasks (plus today's done tasks for the Board's last column).
- **Board** — the status columns: **Inbox · Todo · Standby · Done today** (done today = completed on the current local day; press the `done` pill to see all done tasks instead). On a wide screen (≥ 1024 px) the columns sit side by side using the full width, each scrolling on its own under a sticky header with its count; on the phone they are a one-column scroll-snap carousel and the count strip above doubles as the column switcher (the fleet launcher's board layout, ported). Flat regions split by 1px hairlines. Status pills hide the columns not selected. **Drag a row onto another column** to change its status (`PATCH`, an activity row, a toast on error) or use the row's select.
- **Select several tasks at once** (#81) — the checklist toggle in the Board's and the Table's top strip turns every card/row into a tick target (drag is off while it is on, and a tap on the phone selects rather than opening — the horizontal swipe still belongs to the Board's carousel). It is **one selection**: tick three cards on the Board, switch to the Table, and the same three are ticked there. With at least one ticked, the bulk bar takes the top strip over, on **one line** with every control at the strip's own square height — *N selected* · **set status** (`complete` included, so a recurring task in the selection rolls its due instead of closing, exactly as its own row select would) · **set due** (the native date picker; the natural phrases stay where you type — quick-add, the drawer, the CLI and the API) · ✕ to leave. One `POST /api/tasks/bulk` per action, applied through the same repo layer as a single-task edit, so every task gets its own activity row. A task that fails — deleted in another tab, say — is **named in the toast** (`1 updated · 1 failed (#36: …)`), never dropped in silence; Escape leaves Select mode. The bar's fourth square is **delete** (#121): one confirmation names the count (and how many are projects whose children go too, and how many are synced coding tasks the next sync would bring back), then `POST /api/tasks/bulk/delete` with the same per-id report.
- **One key per action** (#99) — with a row focused, `e` completes, `1`–`3` set the status, `t` / `w` set the due date, `s` snoozes, `p` cycles priority, `z` undoes, `?` lists them; with tasks ticked the same key acts on the whole selection. See [Keyboard triage](#keyboard-triage).
- **Done journal** (#102) — everything closed, newest day first, one flat heading per local calendar day (`Yesterday · Tue 1 Sep · 2 done`) over the same rows (project, issue chip, the status select that reopens a task in place); cancelled tasks ride along muted, with a switch in the head to drop them. A reading surface, not a tab: open it from the command palette (*Journal*) or the **journal** link in the Board's Done column head — `#journal` deep-links it (`#journal/task/42` with the drawer open), the shared filter card applies and rides the URL (project · person · text; status, due, modified and sort are hidden because they say nothing about a closed task), and any tab press leaves. The page is this week; **Show the week before** widens it seven days at a time, and the journal only says it has reached the end once the server confirms nothing closed earlier. Friday afternoon it answers "what actually happened this week"; the weekly review will quote it. `done_at` is the **closed-at** stamp — set when a task enters `done` *or* `cancelled`, cleared when it reopens (schema v10 backfills cancelled tasks from their activity log) — and the day is the local calendar day, the same rule as the Board's *Done today*. Terminal: `tasks journal [--weeks N]`.
- **Today** — tasks due ≤ today (overdue first, then today) grouped by root project, sorted by due date within each group (same as every other view — a schedule change always reorders it), the same rows (status changed through the select — a recurring task also gets a **complete** option that rolls its due one cadence forward instead of closing; **done** always closes for good, recurring or not); group titles in the app's normal title font, a flat heading line with the overdue / due-today counts. Each row also carries the **snooze** control — *Tomorrow · This weekend · Next week · Pick a date…* — which sets the task's start date and takes it off the list, with an Undo in the toast (see [Start dates & snooze](#start-dates--snooze)). **My plan** sits on top — the tasks committed to today, ordered, with the *n of m done* progress line and the plan-your-day banner when the plan is empty (see [Plan my day](#plan-my-day)); a task planned today lives there instead of in the due groups. **Later this week** (tomorrow … +7 days) sits collapsed below as a flat disclosure. This is the phone's landing tab: on a first visit a touch device opens on Today, the desktop on the Board (the last tab is remembered after that).
- **Table** — the same list as a flat full-width grid on a wide screen: code · title (breadcrumb `Parent › Child` under it) · due (relative, ISO on hover, overdue in the danger tone) · status (the same select) · priority · person · project (the top ancestor) · folder chip · last comment (links rendered as chips) · next action. Click the due cell and the date picker opens on the spot (#107) — pick a day and it is saved. Row click / Enter opens the drawer. In Select mode the grid grows a leading checkbox column of its own (the shared row's checkbox belongs to the phone rendering). Under 768 px the grid has no room, so the Table renders the shared rows — identical to the Board's. The tab's top strip carries a **two-state view toggle** (#161) — grid or **Tree** — because both are drawings of the one filtered list over one strip and one filter card; the nav pill is for destinations, so the Tree stopped being a tab and became the Table's second view. The choice is remembered (`task-os.tableView`), so an installed PWA reopens where you left it.
- **Tree** — the Table tab's other view: the whole hierarchy as an outliner, indent = nesting with a subtle guide line per depth, collapse/expand persists, every node with children carries a **project** chip and a rollup (open descendants, nearest due). Every level orders the same way: open before closed, then by due (none last), then title. On a fine pointer, drag a row onto another to re-parent it (`POST /api/tasks/{id}/move`; a cycle is refused with a toast) — a dashed top-level drop zone appears while a drag is live; on a touch device there is no drag at all: re-parent from the drawer's **Move to** field. Keyboard: ↑/↓ walk, →/← expand/collapse, Enter opens. It is also the one working view that still shows a **deferred** or **blocked** task, with the marker saying why the others are quiet about it.
- **Task drawer** — a right-hand side panel on desktop (≥ 1024 px, the list stays visible beside it), full-screen on the phone; deep-linkable as `#task/42`. Breadcrumb (clickable) → editable title → fields (status, priority, due, starts, repeat, person, code, move-to — projects + top level, the re-parent path that needs no drag) → description (markdown, edit/preview) → links (add/remove; an AI-conversation URL is recognised and wears the bot chip — see [AI conversation links](#ai-conversation-links)) → comments newest-first with URLs / `{folder}` refs / `repo#N` as clickable chips + a composer (Ctrl+Enter sends, `origin = ui`); every comment carries the same two icons the link rows do (#155) — a **pencil** that opens the body in place (Ctrl+Enter saves, Escape cancels; the author, time and origin are untouched, an edit corrects the text and never re-attributes it) and a **trash** that asks first, since there is no undo → activity log (`field old → new · actor · time`) → children (click to navigate, add child) → **issue panel**: the linked issue (provider glyph, `repo#N` chip, state pill, labels, last synced, *Sync now*, *Unlink*) or, for a plain task, *Create issue* (repo from the last-seen list or typed) and *Link existing* (`owner/repo#N` or the URL). See [Issues as tasks](#issues-as-tasks). Last, quiet, at the foot: **Delete task** (#121) — a confirmation names the task and, when it has one, its subtree (*Its 3 child tasks go with it*); confirm and the task is gone with its comments, links and history (no trash, no undo — the dialog says so), the drawer closes, every view refreshes, a toast says how much went. A **synced coding task whose issue is still open** gets a warning in the same dialog: the next issue sync recreates it while the issue stays open, so unlink it first or close the issue — the delete is still yours to confirm, but never a silent resurrection ten seconds later.
- **Archive** — the sixth destination and the only one that is not a rendering of the task list: the head carries the *Archive Inbox now* button, a **first N mails** bound (empty = the whole Inbox), the archiver's own state (configured / running / the model it ranks with / the last run / *not configured* with the reason) and what the runs on the picker add up to. Under it, the chosen run as a full-width grid — subject · from · sent · destination (the last three path components, the full path on hover) · files (each `.msg` a chip that opens through the [per-PC opener](#folders-that-open-on-any-pc)) · confidence · why · state — and per row exactly the review levels that row's state allows: **Accept** (a mail that needed a human), **Move to… / File it…** (the candidates the run already ranked, or any other folder through the same index picker the drawer uses, with an optional one-line hint the next run remembers), **Retry** (#174 — on the one row whose file was written before Outlook refused the move: one tap re-sends the same decision and the mail leaves the Inbox) and **Revert** (the files are deleted and the mail goes back to the Inbox). A run picker lists earlier runs; *Accept all N that need you* clears a reviewed batch in one go; every action that touches a file asks first. The phone gets the same report as cards with the actions behind each row's *Review* — and the whole pane is drawn for 390 px (#168): the head is one line with the three readings one line each under it (bold lead word, muted tail, the whole reading in the title), the run picker reaches the right edge with the bulk button full width under it as *Accept all (N)*, every review level wears the one `.archive-action` button shape, the *File it…* panel puts the folder field's glyph inside its own right edge over a full-width *Archive here*, and nothing — a long subject, a deep path in a reason — can make the pane scroll sideways. The Board's Inbox header carries a quiet **N mail(s) need you** link into the tab when the last run left something undecided. See [Archive the Inbox](#archive-the-inbox).
...
| `capture` | `email_poll_minutes` — how often the flagged-email poller reads the archiver index named by `search.email_db` (default 10; `0` = off, visibly). No path of its own: capture and search read the one file. See [Capture into the Inbox](#capture-into-the-inbox) |
| `voice` | `transcribe_url` — where a recorded quick-add line is forwarded (default `http://127.0.0.1:8000/v1/audio/transcriptions`, the [local-llm-hub](https://github.com/ferraroroberto/local-llm-hub); it names no model, so the hub applies its transcribe role — parakeet first); blank = voice off, visibly. `fallback_url` — the local whisper server, tried **only** when the hub cannot be reached at all (default `http://127.0.0.1:8090/v1/audio/transcriptions`; blank = no fallback). `partial_interval_seconds` — how often the page re-posts the take so far so the transcript appears while you speak (default `1.5`; `0` = one transcription on stop). See [Voice quick-add](#voice-quick-add) |
...
| `calendar` | the read-only calendar lane on Today (#96): `ics_url` — a **private** ICS address (Google's *secret address in iCal format*, Outlook's published calendar link; `webcal://` is read as `https://`); blank = the lane says *No calendar connected*. **It is a secret** — anyone holding it reads the calendar — so it lives in this gitignored file only, and the lane, Settings, `/api/status`, errors and the log show its host alone. `refresh_minutes` (default 10; a failed fetch is retried after a minute), `timeout_seconds` (default 2 — the bound on a fetch and on how long Today ever waits for one). See [Calendar beside Today](#calendar-beside-today) |
...
One SQLite file (`data/tasks.db`, WAL, FTS5), migrated by `src/schema.py` (`settings.schema_version`, currently **16**):

`tasks(id, parent_id, code, title, type task|coding|note, status inbox|todo|standby|done|cancelled, priority high|medium|low|none, due, starts (v7 - the day it starts mattering; see [Start dates & snooze](#start-dates--snooze)), planned_on + plan_order (v8 - the day it was committed to and its position in that day's plan; see [Plan my day](#plan-my-day)), recurrence daily|weekly|monthly|quarterly|yearly + recurrence_anchor (v9 — the fixed day a weekly/monthly recurrence lands on) + recurrence_interval (v16 — repeat every N cadences, NULL = every one; both: see [Start dates & snooze](#start-dates--snooze)), description, folder_ref, next_action, person_id, created_by, created_at, updated_at, done_at, external_id)` · `links(task_id, url, label, kind web|folder|email|issue|ai)` (v5 adds `ai` — an AI-conversation link, see [AI conversation links](#ai-conversation-links)) · `comments(task_id, author, ts, body, origin ui|cli|md|notion|import|sync, external_id)` · `activity(task_id, ts, actor, field, old_value, new_value)` · `people(name, email, avatar_path, external_id)` · `issue_refs(task_id, provider, repo, number, state, url, last_synced)` · `mirror_state(task_id, path, exported_at, file_mtime_ns, content_hash)` (v4 — what the markdown mirror last wrote per task) · `mirror_events(task_id, kind conflict|rejected, field, file_value, kept_value, ts)` (v6 — a mirror import conflict/rejection, deduped on task+field+file value; issue #84) · `task_blocks(blocker_id, blocked_id)` (v11 — a *blocks* edge, cascade-deleted on either end; see [Blocked-by dependencies](#blocked-by-dependencies)) · `ai_suggestions(task_id, payload, status pending|accepted|rejected, created_at, resolved_at)` (v12 — at most one pending suggestion per task; see [AI Inbox triage](#ai-inbox-triage)) · `archive_runs(started_at, finished_at, status running|done|failed, planned, archived, needs_review, failed, error)` + `archive_items(run_id, message_id, entry_id, subject, sender, sent_at, attachments, candidates_json, chosen_folder, chosen_rank, confidence, reason, date_prefix, files_json, sequence, status archived|needs_review|failed|reverted|moved, error, decided_at)` (v14 — one row per archiving run and per mail it decided on, with a partial unique index on `message_id` over the two filed states so a mail is filed once; see [Archive the Inbox](#archive-the-inbox)) · `archive_corrections(item_id, message_id, subject, sender, suggested_folder, chosen_folder, hint, created_at)` + `archive_runs.agreement` (v15 — every time a human overruled the folder ranking, copied rather than joined so deleting a run never deletes what it taught, and the run's share of picks that matched the archiver's own top; see [Archive the Inbox](#archive-the-inbox)) · `tasks_fts` + `comments_fts` (FTS5, trigger-synced).

Rules the repo layer enforces (`src/tasks_repo.py`): a task with children **is** a project (the `project` filter means "descendant of"); `move` refuses cycles; `add_blocker` refuses a self-block and a cycle in its own separate `task_blocks` graph (see [Blocked-by dependencies](#blocked-by-dependencies)); every due / status / parent / priority (and title, type, recurrence, person, description) change writes an `activity` row, and so does every blocker add/remove, on both tasks; `POST .../done` on a recurring task rolls the **same** task's due to the next occurrence after both that due and today (the `recurrence_anchor`'s fixed day when it has one, every `recurrence_interval`-th cadence when it has one; month-end clamps) and logs the completion, otherwise it sets `done` + `done_at` (the web app's status select reaches this via a recurring task's **complete** option; picking `done` there is always a plain status change — closed for good, recurring or not); `done_at` is the **closed-at** stamp — `cancelled` sets it too and reopening clears it (v10 backfilled older cancelled rows from their activity log), which is what the [done journal](#views) groups by; `type = coding` **iff** an `issue_refs` row exists (attach an issue to make a task coding — setting the type by hand is rejected); a task whose `starts` day has not arrived, or that still has an open blocker, is hidden from the working views by `list_tasks` (see [Start dates & snooze](#start-dates--snooze) and [Blocked-by dependencies](#blocked-by-dependencies)); planning appends to the day's `plan_order`, snoozing a planned task un-plans it, planning a deferred task wakes it (see [Plan my day](#plan-my-day)); deleting a task deletes its subtree and frees anything it blocked.
...
| `GET /api/board?project=&person=&q=` | `{today, columns: {inbox, todo, standby, done}}` — the Board's buckets, same enriched summaries as the list; `done` = completed on the current local day only |
| `GET /api/today?person=` | `{today, plan: {items, done, total}, due: [{root, items}], week: [{root, items}], counts: {overdue, today, week}, calendar}` — `plan` = the tasks committed to today ordered by `plan_order`, done ones included for the progress line ([Plan my day](#plan-my-day)); `due` = open tasks due ≤ today grouped by root project (sorted by due date within each group), minus what is planned today; `week` = tomorrow … +7 days in the same shape (the CLI's shape; the web app derives the due/week buckets from the filtered list itself and reads only `plan` from here); `calendar` = the [calendar lane](#calendar-beside-today): `{configured, reason, source, state, error, date, all_day: [{summary}], events: [{summary, start, end, starts_before, ends_after}], skipped_recurring, unreadable, fetched_at, failing_since, stale, refreshing}` — `state` is `ok` · `off` · `bad_url` · `unreachable` · `timeout` · `parse_error`, and `source` is the address's host, never the address |
| `POST /api/calendar/refresh` | fetch the calendar now (Settings' *Refresh now*) → the same `calendar` group `/api/today` carries, bounded by `calendar.timeout_seconds` |
...
| `GET /api/ai/suggestions` · `POST /api/ai/triage` `{task_ids?}` | list pending staged suggestions · ask the configured local model for exactly one suggestion per selected/current Inbox task. The entire response is validated before any suggestion is stored; malformed JSON, missing/extra task ids, invalid project/person ids, cycles, priorities or dates reject the whole run |
| `POST /api/ai/suggestions/{id}/accept` · `POST …/{id}/reject` | accept applies the staged parent, priority, due and person with activity actor `ai`, then moves the task to **Todo**; reject leaves the task untouched. Both resolve only that suggestion |
...
| `GET /api/status` | `{https, auth: {enabled, password, client}, mirror: {enabled, dir, files, last_export, last_import, errors, error_files, watching, events, …}, backup: {enabled, dir, last_file, next_run, last_error, …}, folders: {enabled, roots, entries, last_indexed, indexing, stale, reason}, capture: {enabled, reason, source, poll_minutes, last_run, last_result, last_error, next_run, running}, voice: {enabled, reason, url, checked_at}, ai: {enabled, reason, model, checked_at}, archive: {configured, reason, running, repo, candidates, confidence_threshold, timeout_seconds, model, batch_size, examples, last_run, last_error}, opener: {install, uninstall, env_template, installed_here, mode}, placeholders}` — how this request came in (`client`: `loopback` · `token` · `public`) and what the install accepts; a disabled service carries its `reason`. `opener.mode` is `launcher` · `launcher-stale` · `fallback` · `null` — which registration shape this PC uses (see [Folders that open on any PC](#folders-that-open-on-any-pc)). `mirror.events` is the standing import-conflict/rejection count (issue #84) |
...
| `POST /api/capture/email/run` | one flagged-email capture pass now → `{listed, created, unchanged, dismissed, errors, created_ids}` (409 `capture_disabled` + the reason when the poller is off) — see [Capture into the Inbox](#capture-into-the-inbox) |
| `POST /api/archive/run` `{limit?}` (or `?limit=`) | start an archiving run → **202** with the run row in `running`; the work continues in a worker thread, because a full Inbox is walked over COM and takes minutes. `limit` bounds the run to at most that many mails, in the archiver's own plan order — what a first, undoable run uses. `409 archive_disabled` (with the reason) · `409 archive_in_flight` · and, once running, the run row itself carries `502` Outlook-unreachable / `504` timed-out as its `error` — see [Archive the Inbox](#archive-the-inbox) |
| `GET /api/archive/runs?limit=` · `GET /api/archive/runs/{id}` | every run, latest first → `{runs, count}` · one run **with its items** → the run fields (`planned, archived, needs_review, failed, error, agreement` — `agreement` is the share of model-ranked mails filed where the archiver's own suggester would have, `null` when the model ranked none) + `items: [{id, message_id, subject, sender, sent_at, attachments, candidates, chosen_folder, chosen_rank, confidence, reason, date_prefix, files, sequence, status, error, decided_at}]` — `chosen_rank` is the suggester's rank of the pick (0 = the model agreed), `confidence` and `reason` the model's own. Both read the database only, so a past run is still readable on a machine where the archiver is not installed |
| `POST /api/archive/items/{id}/revert` · `POST …/move` `{folder, hint?}` · `POST …/accept` `{hint?}` · `POST …/retry` | undo one mail (delete its files, move it back to the Inbox) · re-file it into another folder (a ref or an absolute path, resolved like `POST /api/resolve`) — an undo followed by a fresh filing · mark a `needs_review` / `failed` row seen, changing no file · finish a `failed` mail whose files are on disk by re-sending the same decision, unchanged (no body; a second refusal leaves the row `failed` with the archiver's newer message and answers `502 archive_retry_failed`). A move that lands, and an accept on a `needs_review` mail that had a folder, both write an `archive_corrections` row the next run's prompt carries; the optional `hint` (≤500 chars) is your one-line note on it. `409 archive_bad_state` when the row is in the wrong state, `404` when there is no such item, `422 archive_bad_folder` for a ref this install cannot resolve, `502` when the archiver refused |
| `POST /api/transcribe[?parse=0]` (body = the audio, `Content-Type` = its type) | the recorded quick-add line → `{text, parse}` — the transcript **and** the `POST /api/parse` split of it, in one round trip; `?parse=0` returns `{text}` alone, which is what the live-transcript pass asks for while you are still speaking. The body *is* the clip (no multipart), and what the browser sends is always 16 kHz mono WAV, which is the only thing either endpoint decodes. `409 voice_disabled` (no `voice.transcribe_url`) · `413 audio_too_large` · `422` (empty body) · `502 voice_rejected` (the endpoint answered and refused the clip, with what it said) · `503 voice_unavailable` (it never answered) — see [Voice quick-add](#voice-quick-add) |
| `POST /api/enrich` `{text, today?}` | the spoken line → `{title, description, due, due_phrase, starts, starts_phrase, parent, parent_ref, source, model, reason?}`. **Always 200** (except a malformed `today`, 422): the transcript is already on screen, so a hub that is down, slow or answering junk comes back as the deterministic [quick-add parse](#api) with `source: "parser"` and the reason — never an error the user reads as a broken microphone. `source` is `llm` or `parser` and is never omitted. See [Voice quick-add](#voice-quick-add) |
...
`tasks.bat` at the repo root runs `src/cli.py` in the venv. It talks to the running app when `:8448` answers — `https://127.0.0.1` first when the cert pair exists (the tray then serves TLS; loopback skips the hostname check, since the leaf names the tailnet host and loopback is already the owner), else plain `http://` (#259); `--server URL` / `TASKOS_URL` point elsewhere, verified normally and falls back to opening the database directly when the app is down (`--local` forces that). `--json` on every command prints the same shapes the API returns; `--actor NAME` sets who is acting; `-v` says which backend answered.

```
tasks add "title" [--parent N] [--due <date>] [--starts <date>] [--priority high|medium|low|none]
                  [--status inbox|todo|standby|done|cancelled]   default todo
...
tasks triage               stage AI suggestions for every Inbox task, then y/n/q each one
tasks plan ls              today's plan, ordered, with the n-of-m progress line
tasks journal [--weeks N] [--project N] [--person id|name]   what got done, grouped by day, newest first (this week; N weeks back)
tasks move N --parent M    M = root for top level; cycles are refused
tasks block N --on M       N is blocked by M — refused on a self-block or a cycle
tasks unblock N M          remove M as a blocker of N
tasks rm N [--yes]         delete N and its subtree — names what goes and asks [y/N] on stderr (a refusal is exit 1, so a script that forgot --yes fails loud); alias: delete
tasks search "q" [--kind tasks|folders|emails|issues]   federated: one block per kind; unconfigured indexes say so
tasks people
tasks mirror [export|import|status]   markdown mirror: full export · one watcher pass · status (default; also the backup, folder-index, capture and voice lines)
...
**Recurrence:** the drawer's **Repeat** field is a cadence — `daily · weekly · monthly · quarterly · yearly` — then an **Every** count for how many of that cadence to wait (#229; empty or 1 is every one), and, for weekly and monthly, an **On** picker for the fixed day it lands on (#112):
...
**Every N** composes with any cadence, anchored or not — *every 3 days*, *every 2 quarters*, *every 2 months on the 15th*, *every 7 weeks on Saturday* — and is stored beside the cadence in `recurrence_interval` (`null` = every one). An anchored interval takes its rhythm from the **due date**: that due's week (Monday-first) or month counts, and so does every Nth one after it. The roll lands on the anchored day in the next qualifying week or month, so completing early, on the day or weeks late never shifts the rhythm, and editing the due by hand deliberately re-sets it. A due that is not on the anchored day settles onto it within its own week first (as without an interval), and steps whole intervals from there. Clearing Repeat clears the interval; switching cadence keeps it.
...
tasks add "Treat clothes against moths" --due sat --recurrence weekly --recurrence-anchor sat --recurrence-interval 7
```

## Blocked-by dependencies

A task can name another task as its **blocker** — "Order tiles" can't happen until "Confirm measurements" is done. While any blocker is still open, the dependent is **blocked**: absent from the working views (Board, Today, Table, `tasks ls`, plan candidates) exactly like a deferred task, present everywhere that claims to show everything (the Tree, search, `tasks show`, `tasks ls --status all`) wearing a lock glyph and a **blocked by N** count, and back on its own the moment the last open blocker closes — a pure query, no event machinery. A closed blocker (done or cancelled) doesn't count: only an *open* one gates.

Blocked is its own edge graph (`task_blocks`), separate from the parent tree, so a blocker may sit anywhere — a sibling, a cousin project, unrelated entirely. Adding one is refused with a `cycle` (409) on a self-block or on anything that would make the two tasks wait on each other, the same guard shape as `move`'s parent-tree cycle check but walked over its own graph. Deleting a blocker task frees whatever it blocked and logs a `blocked_by` old→None row on the survivor, so the log says why it woke up.

**Blocked wins over deferred.** A task that is both asleep (`starts` in the future) and blocked shows the lock, not the clock — it's the harder gate, and there's only one marker's worth of room on the row.

The drawer's **Blocked by** section lists the open and closed blockers (click to navigate, a trash icon to remove) and a picker to add one — the same "Move to" shape, searching every task rather than only projects. The filter card's status multi-select carries a **`blocked`** entry (a modifier, not a status, exactly like `deferred`): `?status=blocked` is the locked tasks, `?status=blocked,todo` the locked ones that are `todo`.

```powershell
tasks block 12 --on 7      # 12 is now blocked by 7 — refused on a cycle
tasks ls --blocked         # exactly the blocked tasks
tasks unblock 12 7         # 7 is no longer a blocker of 12
```

The markdown mirror carries it as `blocked_by: [7, 9]` in the frontmatter — an inline list of task ids, diffed against the stored edges on import and applied add/remove through the same cycle-guard rejection path `parent` gets (a rejected edit becomes a `mirror_events` row, never a comment).
...
## Calendar beside Today

On the desktop, Today carries a slim **Calendar** lane beside the task list — today's events from your real calendar, read-only, so the plan is made against the free time that actually exists (plan mode keeps it alongside the candidates). All-day events sit on a strip at the top; timed ones are listed with their local times, a meeting that runs past midnight says *ends tomorrow*. Nothing is ever written to the calendar. The phone skips the lane for now.

Point `calendar.ics_url` at a private ICS address and restart. The feed is read with the standard library: all-day and timed events, UTC / floating / `TZID` times (the feed's own `VTIMEZONE` first, so Outlook's Windows zone names work), cancelled events, `EXDATE`, `RDATE`, and a moved occurrence of a recurring meeting (it shows once, where it moved to). Recurring rules are expanded when they are daily or weekly (`INTERVAL`, `UNTIL`, `COUNT`, `BYDAY`); anything else — monthly, yearly, *the first Monday* — is **counted, never dropped**: the lane says *1 recurring event could not be checked* rather than implying a free afternoon.

**An empty lane never means a free day by accident.** *No events today* is drawn only after a good fetch; every failure is its own sentence with the reason and since when — *No calendar connected* (blank address), *Calendar address refused* (not an http(s) address, or a 4xx — revoked or mistyped), *Calendar unreachable* (no connection, a dropped one, a 5xx), *Calendar did not answer in time* (nothing complete within `timeout_seconds`) and *Calendar feed unreadable* (an answer that is not a calendar, like a sign-in page). A failure after a good fetch keeps that copy on screen, marked with the failure and *Showing the copy from 09:00*. The same state is in Settings → **Calendar** (with *Refresh now*) and `GET /api/status` → `calendar`.

**Today never waits on it.** A good copy is served for `refresh_minutes`; once it expires, Today answers at once with the copy marked *refreshing* while one background fetch replaces it. Only with nothing to show yet does a request wait for the fetch, and never longer than `timeout_seconds`.
...
| `1` `2` `3` | status inbox · todo · standby |
...
- **frontmatter fields** `title`, `code`, `status`, `priority`, `due`, `starts` and `planned_on` (ISO or a natural phrase — `tomorrow`, `next friday`, `oct 1`), `recurrence` + `recurrence_anchor` + `recurrence_interval`, `person` (by name), `folder_ref`, `next_action`, `parent` (id; the cycle guard applies) and the **`## Description`** body are applied through the same repo layer as the UI, with **actor `md`** — every change lands in the activity log like any other (`plan_order` is deliberately **not** in the file: presentation-level ordering, and mirroring it would churn every synced file on every drag);
- **new lines under `## Comments`** become comments with `origin = md`: a bare `- your text` (author = the first `team.people` entry, time = now) or a full `- <ISO ts> · <author> · md: text` line; a body may continue on lines indented by two spaces. Comments are **append-only** *on this path* — a deleted line is not a deletion, an edited existing line reads as a new comment (the original stays). Editing and deleting a comment is the drawer's pencil and trash (#155); either write re-exports the file, so the next import sees the corrected thread rather than resurrecting the old line;
...
**Provenance rule** (#126) — the file's `exported_at` is the one line no human edit touches, so it says whose rendering a file is. Equal to what the app last wrote: its own file, edited. Older: a stale copy of its own file (a sync client restoring a previous version) — still an edit, judged against *that* snapshot, so anything the database changed since it wins. **Newer: the app never wrote it** — another task-os instance rendered *its* database into the folder (a fresh checkout on the sample config, a test harness pointed at the real folder). That is not an edit: nothing is applied, every differing value becomes a `conflict` event (`import conflict on person: file said ∅ (written by another task-os instance …), kept Alex Chen`), the file is re-exported to canonical — or, under a name the app never wrote, skipped and listed under `error_files`. Two boundaries keep a disposable instance out of the real folder in the first place: an instance on an overridden database (`TASKOS_DB_PATH`) only mirrors and backs up to a folder **beside that database** (anything else is refused with the reason in `/api/status`), and the committed sample config never enables either service — a checkout without its own `config/config.json` boots with the mirror and backup off.
...
What gets registered is `opener.ps1`, which receives the URL as an **argument**, wrapped in `conhost.exe --headless` so the console `ShellExecute` would otherwise allocate is never visible ([`opener/README.md`](opener/README.md#why-headless-task-os130) has the probe table — plain `powershell.exe`, even `-WindowStyle Hidden`, still flashed a Windows Terminal window on the PC it was measured on). Pointing the scheme straight at the `.cmd` instead would hand it to a command interpreter as a *string* that is re-parsed, so a quote inside the URL could start a second command — measured against every such shape on Windows 11 and none of them safe ([`opener/README.md`](opener/README.md#why-a-launcher-and-not-the-handler-directly) has the grid). Where a machine policy blocks running a script file the installers fall back to the old `.cmd` registration and **say so** (`mode: FALLBACK`, `FALLBACK mode` at the end of the pasted line, a `fallback mode` chip in Settings) — a degraded install is never silent, and a PC still registered with the pre-#130 visible shape gets a `launcher-stale` chip pointing at re-running the command. Then click any folder chip: the browser asks once *"Open task-os opener?"* (tick *always allow*), and Explorer opens with nothing else on screen. If nothing opens, a one-time hint appears under the chip pointing at the Settings card. Details, what was verified and the caveats: [`opener/README.md`](opener/README.md).
...
The Board's Inbox header has one **Triage** action. It sends the current Inbox tasks, their descriptions, the open project tree and people list to the configured [local-llm-hub](https://github.com/ferraroroberto/local-llm-hub) in one request. The model proposes a project (or top level), priority, due date (or none) and person (or unassigned) for every task. Nothing on a task changes yet: each proposal is stored as a pending `ai_suggestions` row and appears directly under its task with an individual ✓ / ×. Accepting applies all four proposed fields with activity actor `ai` and moves that task to **Todo**; rejecting discards only that proposal. There is deliberately no accept-all and no automatic application.

The response is an all-or-nothing contract. It must be JSON with exactly one result for every requested task and no others; every task, project and person id must exist in the supplied context, priority and date must be valid, and a proposed parent must not form a cycle. Any malformed or incomplete answer stages **nothing**. Re-running Triage replaces the pending batch only after the new answer has passed the whole validation, so a bad retry does not erase suggestions already waiting for review.

`tasks triage` is the same flow in a terminal: it prints each staged proposal and asks `y/n/q`; `--json` stages and returns the API shape without prompting, so scripts never accept work accidentally. One run is capped at 50 Inbox tasks and descriptions are truncated before they reach the model. The hub is called through the Anthropic-compatible `/v1/messages` interface using `ai.model`; there are no automatic retries, and a second run while one is in flight is refused. The Board action, Settings → *AI Inbox triage*, `/api/status` and CLI status all spell out **off** or **AI unavailable** when the feature is disabled or the local hub cannot be reached.

## Capture into the Inbox

Search is *pull* — you go looking. Capture is *push*: something you already marked elsewhere lands in Inbox on its own, so nothing depends on remembering it. Two channels, both idempotent, both **one-way**.

**Flagged emails.** `src/email_capture.py` polls the archiver's index — the same `search.email_db` file the emails adapter searches, opened the same way (`file:…?mode=ro`, a fresh connection per query, never a write, never a migration: the archiver owns that file). Every message carrying an Outlook **follow-up flag** becomes an Inbox task: title = the subject, description = `From email: sender · date` + the flag's own text + the preview, plus a `links(kind=email)` row holding the `.msg` ref — so the existing [opener chip](#folders-that-open-on-any-pc) opens the mail with no new UI. `created_by` is `email-archiver`, so the activity row says which channel brought it in.

The flag is something the **archiver** records while scanning, in two columns this repo only ever reads:

| Column | Source | Meaning |
| --- | --- | --- |
| `flag_status` | MAPI `PidTagFlagStatus` (`0x1090`) | `0`/absent = none · `1` = complete · `2` = flagged for follow-up |
| `flag_request` | `PidLidFlagRequest` | the flag's text ("Follow up"), nullable |

> **Not shipped yet.** `email-archiver` does not record these columns today, so on a current install this card reads **not configured**, naming the missing column. That is the honest state, not an empty result — the poller starts working the moment the archiver ships them, with no change here. The gesture it is built for is *flag in Outlook, then archive as usual*: the index covers archived mail, so a flagged message you never archive is never seen.

**WhatsApp messages.** whatsapp-radar owns its own data and its own UX; when you mark a message it POSTs `/api/tasks` with the message id as `external_id` and the bearer token. task-os just receives.

**Both dedupe on `external_id`**, and that is the whole contract: `POST /api/tasks` carrying one is idempotent — the first call creates (**201**), every replay returns that same task **untouched** on **200**, so a retry after a timeout cannot make a second Inbox task. The email side keys on `email:` + the `.msg` ref folded onto the placeholders (`email:{onedrive}/mail/house/…msg`) rather than the archiver's `emails.id`, which is a rowid that reshuffles if the index is ever rebuilt; a mail *moved* inside the archive changes its ref and so captures once more.

One-way means what it says: **clearing the flag never removes the task**, and no later pass overwrites a title you edited or a status you moved. A captured task, once real, is yours.

**Deleting a captured task dismisses it.** An archived `.msg` keeps the flag it had when it was saved, so the index offers that mail on every pass for as long as the file exists. Every capture key is therefore also recorded in `capture_keys`, which outlives the task: after a delete the pass counts the mail **dismissed** (in the log, the Settings card and the `capture` status line) instead of landing it again, and a `POST /api/tasks` replay of a deleted capture answers **410** `capture_dismissed`. Closing the task instead (done / cancelled) keeps it, and its history, where it is. Nothing is ever written back to the mail or the message, and completing the task does nothing to either.

**Where you see it** — Settings → *Capture into Inbox* (state word `on` · `checked` · `off`, the reason spelled out when off, and **Check now** for a pass on demand), `GET /api/status` under `capture`, and `tasks mirror status`'s `capture` line. `POST /api/capture/email/run` runs one pass and answers **409** `capture_disabled` with the reason when the poller is off — never a silent no-op. One pass lands at most 200 tasks so a first run against a large archive cannot flood Inbox; the rest arrive on the following passes.

## Archive the Inbox

The last manual chore: filing the Outlook Inbox one mail at a time. The **Archive** tab does the whole tray in one press — every mail is written into an archive folder of [email-archiver](https://github.com/ferraroroberto/email-archiver)'s, moved to Outlook's *Archive* folder and tagged, and the run is recorded so you can read what happened and undo any single mail. *I finished my mails, I press Archive, I read what went where, I fix the one or two that are wrong, and I never file a mail by hand again.*

**The screen.** Press *Archive Inbox now* (optionally bounded to the **first N mails**) and the button disables, a progress line follows the run and the report fills in when it lands; a second press while one is going says so, because one Outlook takes one run at a time. Each row then offers exactly the review levels its own state allows — a control that the API would refuse is not drawn:

| Row state | Accept | Move to… / File it… | Retry | Revert |
| --- | --- | --- | --- | --- |
| `archived` · `moved` | – (finished, not reviewed) | into any *other* folder | – (nothing half-done) | deletes the files, mail back to the Inbox |
| `needs_review` | marks it seen (and teaches the folder it had) | files it for the first time — no undo leg | – (nothing on disk) | – (nothing on disk) |
| `failed` | marks it seen | re-files it | when files were written — sends the same decision again and finishes the move | when files were written |
| `reverted` | – | – | – | – (back in the Inbox, the next run owns it) |

A row that has already had its human stops asking for one (#168): a `needs_review` mail you accepted reads **reviewed**, offers no *Accept* and no *File it…*, and says *nothing left to do* where its buttons were — so *Accept all (N)* counts exactly the rows still offering *Accept* instead of drifting from them. It is still in the Inbox and the next run owns it.

**A refused move is one tap from done (#174).** `apply` writes the `.msg` *before* it moves the mail, so Outlook refusing the move (`move_failed`) leaves the file on disk and the mail in the Inbox — and every later `plan` then reports that mail `already_archived`, so no run would ever offer it again. Two things close the loop. **Retry** on such a row re-sends the *same* decision — same message, same folder, same naming form, nothing re-ranked — and the archiver finishes it: it writes nothing, reuses the file it already wrote, moves the mail and tags it. The row becomes `archived` keeping its files and the sequence number the first attempt allocated (a reuse allocates none), and the error text stays on screen until that actually happens; a second refusal keeps the row `failed` with the archiver's newer message, which is the one that says whether restarting Outlook is what is left to try. And a **run** that meets a mail the index already has *and* which the plan still reports `in_inbox` finishes it the same way rather than recording it as done — it reads **finished a mail already on disk**, with no rank and no new file. Accepting a `failed` row is not finishing it: seeing a mail does not take it out of the Inbox, so *Retry* stays offered on a row already marked seen.

*Move to…* lists the candidates the run already ranked (no re-plan — they are stored on the row) and *Other folder…* opens the same folder-index picker the drawer and quick-add use. The one-line **hint** beside it is optional and is what the next run remembers. Everything that touches a file asks first.

Everything on the screen is also the API, and reading a past run needs no archiver on the machine at all:

**task-os never imports the archiver.** It spawns that repo's `main_batch.py` in the archiver's own venv and reads back one JSON document — `plan` (read-only, ranks folders for every Inbox mail), then `apply` (files, moves and tags the decided ones), then the archiver's `main_scan.py --no-ui` so its index learns the new files. Every verb drives Outlook over COM, and a COM modal blocks whatever thread raises it, so a stuck Outlook is a child process this app kills on `archive.timeout_seconds`, never a wedged webapp. The archiver keeps sole ownership of Outlook, of the filenames and of its index; task-os only decides *which folder*, and hands the candidate's own `date_prefix` straight back so the destination's naming style is the archiver's call, not this app's.

```powershell
curl.exe -X POST "http://127.0.0.1:8448/api/archive/run" -H "Content-Type: application/json" -d '{\"limit\": 1}'
curl.exe "http://127.0.0.1:8448/api/archive/runs"
```

**Start with `limit`.** One run, one mail, then look at it and undo it — trust in something that moves your mail is built on an undo you have actually seen, not on a promise. `limit` caps the mails a run touches, in the archiver's own plan order, and rides either the JSON body or `?limit=`.

**Which folder is a two-layer decision.** The archiver's suggester is a search engine — full-text match, thread continuity, folder-name similarity — and it cannot read a mail. So the ranked candidates go to the local model (`archive.model` through the same hub `ai.base_url` names), in batches of `archive.batch_size`, and it answers with **an index into that list**, a confidence and a one-line reason. It can never name a folder, invent one, reach outside the archive tree or compute a date: it only picks a number, and the path is resolved here from the archiver's own output. The answer is JSON validated whole — exactly one pick per mail in the batch, every index in range, every confidence a number in [0, 1] — because the hub refuses `response_format: json_schema` on the open-weight models, so the validator *is* the contract. A pick below `archive.confidence_threshold`, or an explicit *none of these fits*, leaves the mail in the Inbox as `needs_review` with the model's own words as the reason.

**It learns from every correction.** A `move` that lands, and an `accept` on a `needs_review` mail that had a folder, write an `archive_corrections` row — subject, sender, the folder the system chose, the folder you chose and your optional one-line `hint`. The last `archive.examples` of them ride the next run's prompt as few-shot lines (*mail "…" from … → suggested A, chosen B, hint: …*), oldest first so the block only grows at its tail. Correct it once and the next run of the same kind of mail follows you. Each run records `agreement` — the share of ranked mails the model filed exactly where the suggester would have — so a run whose picks are all rank 0 is visibly a run the model added nothing to, and the accept rate over time is what says when the ranking can be trusted unread.

**A hub that is down does not stop a run.** A batch the model cannot rank — hub off, unreachable, too slow, or answering something the validator refuses — falls back to the archiver's own top candidate for **that batch only**, with the reason on every mail in it ("the local model could not rank this batch (`ai_invalid_response`) — the archiver's own ranking was used") and the classified failure in the run's `error`. Filed by a search engine and filed by a model that read it are different facts, and the report never shows one as the other.

> **Model choice is a measurement, not a preference.** The default is `claude_haiku`: over three live runs of the same seven-mail Inbox it answered validly 3/3 in 29–60 s. `agentic_light_nothink` managed 2/3 — an open-weight model emits its reasoning as ordinary output tokens and one run in three never reached the JSON inside the 6000-token budget — and plain `agentic_light` never finished reasoning at all. Both are one config edit away for an install that wants to stay entirely local, and the degrade path above is exactly what a run with one looks like when it misses. `ai_timeout_seconds` (180) is that request's own bound: `ai.timeout_seconds`' 30 s would call a working model a dead hub.

Each mail ends in one of five states, and the run row counts them:

| State | What it means |
| --- | --- |
| `archived` | filed on disk and moved in Outlook — or already filed per the archiver's index and finished from it, the existing file reused and nothing written |
| `needs_review` | left in the Inbox: the model's confidence was below `archive.confidence_threshold`, it said none of the ranked folders fits, or the archiver ranked none at all |
| `failed` | the archiver refused or broke on this mail, with its own code in `error` (`move_failed` — the files are on disk and the mail is still in the Inbox; `category_failed` — filed and moved, only the tag missing; `not_in_inbox`; `archive_failed`). Still revertible **and retryable** when files were written: `apply` fills them in *before* the move on purpose |
| `reverted` | the files are gone and the mail is back in the Inbox |
| `moved` | re-filed into a folder you picked — an undo followed by a fresh filing |

**A mail is filed once.** Identity is the Internet Message-ID (an Outlook EntryID is rewritten by the very move `apply` performs), and a partial unique index covers `archived` + `moved`, so a second run over a mail this database has already filed writes nothing at all — no row, no child call. A reverted mail is free to be archived again.

Four failures you can tell apart, because the fix differs for each: *not configured here* (409 with the reason — no `archive.repo`, no venv Python, no `main_batch.py`), *Outlook unreachable* (the child's own exit 2), *timed out*, and *a run already in flight* (409 — one Outlook, one child at a time; `revert` and `move` take the same lock). A fifth is deliberately **not** fatal: if the archiver's index rescan afterwards fails, the mails are still filed, so the run finishes `done` and carries the rescan's failure in its `error`. Filed-but-not-confirmed is its own state, never folded into a clean run.

### Renumbering, and the paths it heals

The `NNN` prefix on an archived mail is meant to be browsable: open a folder, read down the list, read the thread in the order it happened. Two ordinary operations break that — a `revert` leaves a hole, and an `apply` always takes the highest number even for a mail older than everything already filed. The archiver repairs it (`renumber`), and task-os asks for the repair on both verbs that cause it: `archive.renumber` (default true) puts `--renumber` on the `apply` and the `revert` it spawns.

**Asking for it means healing after it.** A renumber renames files, the archiver updates its own index and nothing else, and this app stores a `.msg` path in three places — the run item's `files`, the email link's ref on a captured task, and that task's `external_id`. So every rename in the map the archiver returns is applied to all three in the same pass (`apply_renumber_map`), and the triggering row's reason says so: *… · renumbered 4 file(s) in heating*. Without it a revert would delete the wrong file, a chip would open nothing, and the next capture poll would land the same mail a second time under its new name. (The capture key moves even when the task was already deleted, so a dismissed mail stays dismissed under its new name.) The map is read defensively: a document without one means nothing moved (that is every archiver build before the feature), an empty list for a folder means *already in order*, and a folder the archiver **refused** to renumber is logged by name — "nothing to do" and "this folder was left alone" are two different facts.

A map produced outside a run — `main_batch.py renumber --folder …` by hand, or the one-off repair of a folder that was already ragged — goes through the same heal from a file:
...
`--dry-run` prints the rows it *would* rewrite per table and touches nothing; the heal is idempotent, so running it twice is a no-op rather than a mess.

`GET /api/status`'s `archive` key (and `tasks mirror status`) says whether this install can archive at all, what the last run did, and whether one is running now — it is what the tab's head renders and what the Board's Inbox *N mail(s) need you* link reads. An install without the archiver shows the reason and a disabled button, never a dead press. Config changes need a restart — `tray.bat --restart` — like every other block here.

**Where you see it, without opening the tab** — Settings → *Batch email archiving* (#167) reads the same block as one card among the others: a state word in the header (`off` · `on` · `running` · `error` · `NN% agreed` once a run recorded how often the model and the archiver's own ranking agreed), then the archiver checkout, the model with its batch size and how many remembered corrections ride along, the threshold with the candidate count, and what the last run did — planned / filed / need you / failed, its agreement and its error when it has one. Not configured says why, in the same place every other service does. **Run now** starts a run over the whole Inbox from there (the bound lives on the tab, where the report you would undo from also lives) and goes out while one is going; the card re-reads itself every two seconds until the run lands, and a press that arrives after another device already started one gets the API's *one Outlook, one run at a time* back as a toast. *Open the Archive tab* is the way to the report.

## Voice quick-add

Hands busy? Open the add dialog, **click the mic beside line 1** and start talking. The button turns red and becomes a stop square, and the words appear on the line **as they are heard** — say *"buy a new filter for the dehumidifier next week"*, click again, and the line settles with the **Due** field already carrying next week's date, the same [quick-add parse](#api) typing it would have produced, one confirming Enter from a task. Whatever was already on the line is kept: dictation adds to it rather than replacing it. Same gesture on the phone.

Nothing leaves the house: the clip's whole journey is **browser → task-os → [local-llm-hub](https://github.com/ferraroroberto/local-llm-hub)**. The phone reaches this app over its one Tailscale HTTPS endpoint and could not open the hub's port if it tried, which is why `POST /api/transcribe` forwards server-side rather than the page posting to the hub directly.

**Through the hub, not around it.** The request names **no model**, so the hub applies its own `roles.audio.transcribe` chain — **parakeet** on the Mac's ANE first, whisper behind it — and the call lands in the hub's observability ring. That is the fleet rule ("don't duplicate hub functionality in downstream apps") and it is what `voice-transcriber` already does. The local whisper server is the **fallback**, tried *only* when the hub could not be reached at all: an endpoint that answered and refused the clip has told you something, and quietly asking a different engine instead would turn one clear failure into two unclear ones.

**The browser converts before it uploads.** Neither end decodes a browser recording: whisper-server answers a bare `400 Invalid request`, the hub answers `400 audio conversion failed`. Probed live against each: **200** for a 16 kHz WAV, **400** for both formats a browser can record (webm/opus on Chrome, mp4/aac on iOS Safari). So `static/voice.js` taps the microphone through an AudioWorklet (`static/voice-worklet.js`), downmixes to mono, resamples to 16 kHz, lifts a quiet clip and writes a PCM WAV. The server side stays what it should be: a plain HTTP forward — no transcoding, no ffmpeg subprocess, no new dependency.

**The transcript arrives while you speak.** Every `voice.partial_interval_seconds` (default 1.5; `0` turns it off and transcribes once on stop) the page re-encodes the take *so far* and posts it to `POST /api/transcribe?parse=0`, replacing the provisional line. This is what `voice-transcriber`'s rolling worker does, minus its machinery: that app needs a session, chunk uploads and an SSE stream because its audio arrives as webm and has to be transcoded server-side by ffmpeg — this one already encodes WAV **in the browser**, so a partial is just an earlier prefix of the same buffer and needs no new route, no session and no streaming. It is affordable because parakeet answers a short clip in a fraction of a second. Partials carry **no parse**: a due date appearing and moving while the sentence is still landing is noise, so the dates and chips wait for the final pass, which is always the source of truth.

Capturing raw samples rather than a `MediaRecorder` blob is what makes that possible at all — a recording container cut off mid-take does not reliably decode (iOS Safari's fragmented mp4 certainly does not), while a complete WAV can be written from any prefix of the sample buffer. It also takes one decoding step *out* of the pipeline.

**The spoken sentence becomes a title and a description.** A sentence you say carries more than a title — *"urgent, I need to call the plumber about the leaking radiator in the guest room before friday"* is a title, a description and a date. Once the transcript has settled, the page asks `POST /api/enrich`, which sends it to the hub's light model (`enrich.model`, default `agentic_light_nothink`) and comes back with a short imperative title and the rest as a **Description** — both landing in the fields where you can still change them. Typing is untouched: a typed line keeps its instant deterministic parse, so `POST /api/parse` and the CLI still agree on what a phrase means.

**The model may point at a date; it may not pick one.** Asked live for *"before friday"* with today Monday 2026-09-07, it answered `2026-09-13` — a **Sunday**; Friday was the 11th. So it is asked for the *phrase*, and two guards stand behind that: the phrase must appear **verbatim** in the transcript or it is dropped, and `src/dates.py` — not the model — turns it into a day. A model that returns a resolved ISO date instead has that date discarded. The cost is a date occasionally missed; the cost of the alternative is a task confidently due on the wrong day.

**Schema-constrained decoding is not available here**, so the JSON is asked for in the prompt and validated in `src/enrich.py`: an OpenAI `response_format: json_schema` request to this model comes back `400 Failed to initialize samplers: Unexpected empty grammar stack after accepting piece: <think>` — its template injects a thinking prefix the grammar cannot accommodate, and `agentic_light_nothink` (the same llama-server) fails identically. The reply is stripped of any `<think>` block and any code fence, parsed, and read through an allow-list of the four keys the form has.

**Losing the tidy-up is not losing anything.** Every failure — no `enrich.url`, the hub down, a timeout, a refusal, junk instead of JSON — leaves the transcript and its deterministic parse exactly where they were, reported as `source: "parser"` with the reason. Nothing is blocked on it: it takes about 1.5 s warm, and the line is already usable before it starts.

**Off is a visible state, never a dead button.** No `voice.transcribe_url`, no endpoint answering (the probe is one cached TCP connect per endpoint — port 8090 is shared with the fleet's other transcriber, so a connect that never comes back does not say *which* of "down" and "busy" it was, and the reason names both rather than picking one; nothing is ever retried in a loop), a browser with no AudioWorklet, a page that is not a secure context, a refused microphone: each leaves the mic visible and disabled with its own sentence under the field. The same sentence appears in Settings → *Voice quick-add*, in `GET /api/status` under `voice`, and in `tasks mirror status`'s `voice` line — as does **which** endpoint is serving (`hub` or `local whisper (fallback)`), so "am I getting parakeet, or the CPU fallback?" is answerable without reading a log. Each transcription also logs what actually served it, from the hub's own `x-hub-served-model` header.

Out of scope for now: dictating descriptions and comments directly, enriching *typed* lines, voice through the global hotkey window, translation.
...
| Notion | task-os |
| --- | --- |
| `status` not started · In progress · Done | `todo` · `todo` · `done` (+ `done_at` = page `last_edited_time`) |
...
- **Signing in.** A teammate opens the URL → the same sign-in card → types the **team password** → a 90-day `taskos_team` cookie (HttpOnly, `Secure` over HTTPS). It is **not** the owner's token — it grants the same access to the app while valid, but it is derived from the token and the team password hash, so rotating either (`gen_token.py --force`, or a new team password) signs every teammate out at once. It is one shared sign-in: there is no revoking a single teammate short of that rotation. With no token configured there is no team sign-in either — the gate stays closed. `set_team_password.py --clear` turns the team password off.
