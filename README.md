...
| `voice` | `whisper_url` — the whisper endpoint the app forwards a recorded quick-add line to (default `http://127.0.0.1:8090/v1/audio/transcriptions`, the fleet's whisper server); blank = voice off, visibly. See [Voice quick-add](#voice-quick-add) |
...
Nothing leaves the house: the clip's whole journey is **browser → task-os → the fleet's whisper server**. The phone reaches this app over its one Tailscale HTTPS endpoint and could not open `127.0.0.1:8090` if it tried, which is why `POST /api/transcribe` forwards server-side rather than the page posting to whisper directly.

**The browser converts before it uploads.** whisper.cpp's `whisper-server` decodes WAV and answers a bare `400 Invalid request` to everything else — a probe against a live `:8090` took **200** for a 16 kHz WAV and **400** for both formats a browser can record (webm/opus on Chrome, mp4/aac on iOS Safari). So `static/voice.js` decodes its own recording with the Web Audio API, downmixes to mono, resamples to 16 kHz and writes a PCM WAV. The server side stays what it should be: a plain HTTP forward — no transcoding, no ffmpeg subprocess, no new dependency.

**Off is a visible state, never a dead button.** No `voice.whisper_url`, a whisper server that is not answering (the probe is one cached TCP connect — port 8090 is shared with the fleet's other transcriber, so a connect that never comes back does not say *which* of "down" and "busy" it was, and the reason names both rather than picking one; nothing is ever retried in a loop), a browser with no `MediaRecorder`, a page that is not a secure context, a refused microphone: each leaves the mic visible and disabled with its own sentence under the field. The same sentence appears in Settings → *Voice quick-add*, in `GET /api/status` under `voice`, and in `tasks mirror status`'s `voice` line.
