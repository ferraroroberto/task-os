...
app/webapp/routers/       misc (shell, /healthz, /api/version) · auth (/login, /api/login|logout, /api/status)
                          · tasks (/api/tasks…, /api/activity) · people · search · views (/api/board, /api/today)
...
| `GET /api/status` | `{https, auth: {enabled, password, client}}` — how this request came in (`loopback` · `token` · `public`) and what the install accepts |
