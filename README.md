...
**Phone / tablet:** no Explorer to open — tapping the chip shows the resolved path with a **Copy** button, plus **Open web link** when there is a web twin: an explicit folder link on the task, or the URL derived from `config.web_roots` when the ref's placeholder has a cloud web equivalent (`{"onedrive": "https://<your-cloud>/…"}` → `{onedrive}/house` opens `https://<your-cloud>/…/house`).
