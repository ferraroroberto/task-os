...
src/                      schema.py (versioned migrations) · db.py (get_db, WAL) · tasks_repo.py (domain rules + write hooks)
                          ...
                          staged Inbox triage validation/application) · logger.py · static_versioning.py
...
| `GET /api/archive/runs?limit=` · `GET /api/archive/runs/{id}` | every run, latest first → `{runs, count}` · one run **with its items** → the run fields + `items: [{id, message_id, subject, sender, sent_at, attachments, candidates, chosen_folder, chosen_rank, confidence, reason, date_prefix, files, sequence, status, error, decided_at}]`. Both read the database only, so a past run is still readable on a machine where the archiver is not installed |
| `POST /api/archive/items/{id}/revert` · `POST …/move` `{folder}` · `POST …/accept` | undo one mail (delete its files, move it back to the Inbox) · re-file it into another folder (a ref or an absolute path, resolved like `POST /api/resolve`) — an undo followed by a fresh filing · mark a `needs_review` / `failed` row seen, changing no file. `409 archive_bad_state` when the row is in the wrong state, `404` when there is no such item, `422 archive_bad_folder` for a ref this install cannot resolve, `502` when the archiver refused |
...
The last manual chore: filing the Outlook Inbox one mail at a time. `POST /api/archive/run` does the whole tray in one go — every mail is written into the archive folder [email-archiver](https://github.com/ferraroroberto/email-archiver) ranks best, moved to Outlook's *Archive* folder and tagged, and the run is recorded so you can look at what happened and undo any single mail. This is the backend (issue #157); the local model picks the folder in #158, and the screen arrives in #159. Until then the trigger is the API.
...
| `needs_review` | left in the Inbox: the best folder scored below `archive.confidence_threshold`, or the archiver ranked none |
