...
| `POST /api/capture/email/run` | one flagged-email capture pass now → `{listed, created, unchanged, errors, created_ids}` (409 `capture_disabled` + the reason when the poller is off) — see [Capture into the Inbox](#capture-into-the-inbox) |
...
One-way means what it says: **clearing the flag never removes the task**, and no later pass overwrites a title you edited or a status you moved. A captured task, once real, is yours. Nothing is ever written back to the mail or the message, and completing the task does nothing to either.
