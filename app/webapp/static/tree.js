 ...
 *
 * Indent = nesting; each node carries a rollup (open descendants, nearest
 * due); a node with children shows the "project" chip — this is where "a
 * task becomes a project" is visible. Collapse state persists per node in
 * localStorage. Drag a node onto another to re-parent it (HTML5 DnD → the
 * caller's onMove → POST /api/tasks/{id}/move; a cycle comes back as a
 * toast). Keyboard: ↑/↓ walk visible nodes, →/← expand/collapse, Enter opens.
 */
...
export function renderTree(host, forest, handlers) {
  ...
  forest.forEach(function (n) { rollup(n); tree.appendChild(buildNode(n, collapsed, handlers)); });

  // Root drop zone: drag here to move a task to the top level.
...
function buildNode(node, collapsed, handlers) {
  ...
  if (hasKids) {
    ...
    node.children.forEach(function (c) { group.appendChild(buildNode(c, collapsed, handlers)); });
