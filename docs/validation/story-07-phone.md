...
4. Board tab: the columns are a **scroll-snap carousel** opened on *Todo*, exactly one column in view; a **swipe** (a scroll of one column width) makes *Standby* the active strip button and the only column in view; the four strip buttons ≥ 44 px, non-overlapping.
...
| 4 Board carousel after the swipe (Standby) | [story-07-phone-3-phone.png](../screenshots/story-07-phone-3-phone.png) |
...
- [ ] **Not verified — needs the owner's phone.** The real-device leg of the story: open `https://<host>.ts.net:8448` in iOS Safari on the tailnet, sign in, *Add to Home Screen*, launch the installed app, walk Today → quick-add → Board swipe → drawer → folder chip. Checklist for the owner:
  ...
  4. Quick-add `call the plumber today` → the row appears; swipe the Board left → the strip advances (*Todo → Standby*); tap a card → the drawer fills the screen; long-press the folder chip → the tooltip / copy fallback shows the `{…}` ref (the opener is Step 9). Screenshot: the drawer.
