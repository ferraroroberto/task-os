...
| 4 | Filter card → status multi-select → **deferred** | Lists exactly the sleeping tasks (the seed's `Book boiler service` + the new one); the URL becomes `?status=deferred`, shareable like every other filter. It is a *modifier*, not a status: ticking `todo` beside it narrows to sleeping `todo` tasks | [3-desktop](../screenshots/story-13-starts-snooze-3-desktop.png) |
