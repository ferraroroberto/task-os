...
| E2e — `test_story_28_calendar.py`, steps 1–9 against two loopback disposable instances and the fake feed | verified 2026-09-13 — passed in the full gate run (16 e2e) and again on its own; its six shots byte-identical across runs (`shot_determinism --check-tree` after a story-28-only run: 0 drifted). The lane on desktop Today and the new Settings card moved 20 existing gallery shots (stories 05, 08, 09, 10, 13, 15, 17, 22, 24, 25); each was reviewed and re-baselined with the owner's approval, and the full gate is green on the re-baselined gallery |
