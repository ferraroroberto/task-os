...
| A3 | One filter card, collapsed by default, shared by every tab, finished items findable, sort included | `filters.js` — `mountFilters()` renders the vendored disclosure into `#boardFilters / #tableFilters / #treeFilters / #todayFilters / #searchFilters`; status pills include `done` and `cancelled`; project · person · due window · **modified window** (new `updated_since` on `/api/tasks`) · text · **sort**; the summary line spells the state out (`todo · Home renovation · sorted by due date · 12 tasks`); state = the URL query on every tab | [desktop board + card open](../screenshots/ux-round-3-8-desktop-board-filters.png); collapsed on every other shot |
