...
| File | What |
| --- | --- |
| `opener.ps1` | The **launcher** — what the URL scheme is registered to run, wrapped in `conhost.exe --headless` (see *Why headless* below) so nothing is on screen. It takes the URL as an argument, refuses one carrying a quote, and hands it to `opener.cmd` through the environment (`TASKOS_OPENER_URL`); a notice (missing handler, empty URL, a refused quote) shows as a popup, and it captures `opener.cmd`'s own output and pops that up too on a non-zero exit. See *Why a launcher* below; the one job it does itself is `taskos://resume?session=…` (#77): it searches `%USERPROFILE%\.claude\projects\**\*.jsonl` for the transcript carrying that web session id and reopens the session in **one** terminal tab (`wt` when installed, else PowerShell) running `claude --resume <local-uuid>` in the repo the transcript records (`"cwd"`); a session this PC never saw opens the conversation on the web instead, with a popup notice. The search walks the transcripts **newest first and stops at the first owner** — the session-url marker sits near the top of a transcript, so a recent session resolves in ~1 s against a 12k-file / 4.5 GB corpus where filtering before sorting cost ~170 s (#227); the bare-id fallback for older transcript shapes is recorded during that same pass, so a miss costs one traversal rather than two. The tab's command is handed over as a single `-EncodedCommand` base64 token, because `;` is Windows Terminal's own new-tab delimiter and it splits on one *before* the quoting of the argument is considered — a `;`-separated `-Command` became three tabs, only one of which had `-d` (#227). `TASKOS_OPENER_PROJECTS` overrides the transcript root and `TASKOS_OPENER_WT=1/0` forces the `wt` branch (tests); dry-run prints `resume: <uuid> in <dir>` + `resume-exec: <argv>` / `resume-web: <url>` and never pops a window. |
...
`tests/test_opener.py` runs `opener.cmd` for real through `cmd.exe`, and `opener.ps1` for real through `powershell.exe`, with the environment pointed at a temp tree (`OneDrive`, `USERNAME`, `LOCALAPPDATA`) and `TASKOS_OPENER_DRYRUN=1`, which makes them print `open: <path>` / `missing: <path>` instead of launching anything (and never pops a window): decoding, placeholder expansion, `opener.env`, the missing-path notice, the quote refusal, that both installers prefer the launcher and announce the fallback, that the registered command wraps `conhost.exe --headless` while the self-test probe stays unwrapped, `registration_mode()`'s `launcher` / `launcher-stale` / `fallback` split, `install_opener.py --dry-run`, and — since `TASKOS_OPENER_DRYRUN` prints the argv it would launch — that the resume tab carries no bare `;`, opens on `-d <repo>`, and decodes back to both echoes plus `claude --resume <uuid>`. Windows-only; skipped elsewhere. The window-visibility and popup-behaviour tables above came from throwaway-scheme probes run by hand — the tests pin the resulting design, they do not re-run the probes, and the "no window" fact itself is verified by a headed walk (`docs/validation.md`), not by the suite.
