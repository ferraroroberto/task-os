"""Quick-add — one line of natural language → title + due + parent reference.
...
def parse(text: str, today: date | None = None) -> dict[str, Any]:
    """Split one quick-add line into ``{title, due, due_phrase, parent_ref}``.

    ``parent_ref`` is ``None``, ``{"id": N}`` or ``{"title": "…"}``; ``due``
    is an ISO date or ``None``. Never raises — an unparseable tail is simply
    part of the title.
    """
    ...
    return {"title": title.strip(), "due": due, "due_phrase": phrase, "parent_ref": parent_ref}
