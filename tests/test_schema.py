...
def test_fresh_db_reaches_current_version(_temp_db: Path) -> None:
    assert dbmod.init_db() == schema.SCHEMA_VERSION == 2
    conn = dbmod.connect()
    try:
...
def test_failed_migration_leaves_previous_version(_temp_db: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    dbmod.init_db()
    monkeypatch.setitem(schema.MIGRATIONS, 3, "CREATE TABLE ok(x); CREATE TABLE ok(x);")  # second stmt fails
    conn = dbmod.connect()
    try:
